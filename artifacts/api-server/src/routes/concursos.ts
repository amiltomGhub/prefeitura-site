...
import { concursosTable, tenantsTable } from "@workspace/db/schema";
...
router.get("/concursos", async (req, res) => {
  try {
    ...
    if (req.query["situacao"]) conditions.push(eq(concursosTable.situacao, req.query["situacao"] as string));
